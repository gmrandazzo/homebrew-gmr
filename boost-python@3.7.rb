class BoostPythonAT37 < Formula
  ...
  def install
    ...
    pyver = Language::Python.major_minor_version Formula["python3"].opt_bin/"python3"
  ...
  test do
    ...
    pyincludes = shell_output("#{Formula["python3"].opt_bin}/python3-config --includes").chomp.split(" ")
    pylib = shell_output("#{Formula["python3"].opt_bin}/python3-config --ldflags --embed").chomp.split(" ")
    pyver = Language::Python.major_minor_version(Formula["python3"].opt_bin/"python3").to_s.delete(".")
    ...
    output = <<~EOS
      ...
      print(hello.greet())
    EOS
    assert_match "Hello, world!", pipe_output(Formula["python3"].opt_bin/"python3", output, 0)
  end
end
